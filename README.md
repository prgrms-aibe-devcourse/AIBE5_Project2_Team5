...
디자이너들이 자신의 포트폴리오를 공유하고,  
클라이언트가 이를 기반으로 디자인 의뢰를 맡길 수 있는 포트폴리오 기반 매칭 서비스.

최근 프리랜서 시장의 성장과 함께 기업 및 개인이 필요로 하는 디자인 작업을 외부 전문가에게 의뢰하는 수요가 증가하고 있다.  
그러나 적합한 디자이너를 찾거나, 디자이너가 자신의 역량을 효과적으로 알리는 데에는 여전히 많은 어려움이 존재한다.

이에 본 플랫폼은 디자이너와 의뢰인 간의 정보 비대칭을 해소하고, 효율적인 협업 환경을 제공하기 위해 설계된다.  
...
### - 카테고리 기반 탐색
라디오 버튼 필터를 통해 로고, UI/UX, 웹 등 카테고리별 디자인을 실시간(Query Parameter 방식)으로 탐색 가능.

### - 폴더형 컬렉션 시스템
단순 북마크를 넘어 사용자가 직접 폴더를 생성하고 관심 있는 포스트를 분류하여 저장할 수 있는 1:N 관계형 아카이빙 기능.

### - 실시간 메시지 인터페이스
대화방 목록과 상세 채팅창을 한 화면에서 제어하는 Master-Detail 레이아웃 기반의 커뮤니케이션 탭.
